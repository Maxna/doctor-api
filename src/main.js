...
$(document).ready(function() {
  $('#showDr').click(function() {
    ...
    $.ajax({
      url: `https://api.betterdoctor.com/2016-03-01/doctors?query=${query}&location=or-portland&skip=0&limit=10&user_key=${process.env.exports.apiKey}`,
      type: 'GET',
      data: {
        format: 'json'
      },
      success: function(response) {
        $('#output').append('<h5>'+`${response.data[0].profile.first_name} ${response.data[0].profile.last_name}`+'</h5>');
